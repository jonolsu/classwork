...
for number in peoplelist:
    print "\n\nExamining person: ", number
    # 
    # TODO: Create a slice called user1 that filters to only include dataset records where the
    # "In" feature (user phone number) is equal to the first number on your unique list above
    #
    user1 = df[df.In == number]
    ...
    weekday = user1.CallDate.dt.dayofweek < 5
    user1 = user1[weekday]
    ...
    #
    before5 = user1.CallTime < "15:00:00"
    user1 = user1[before5]
    ...
    ax.set_title(str(number) + ' Weekday calls before 5PM')
    ...
    ax.scatter([32.7219188],[-96.890399],marker='o',c='blue', alpha=0.5, linewidths=3, s=169)
    showandtell()
    ...
    midWayClusterIndices = clusterWithFewestSamples(kmeans_model)
    ...
    ax.scatter(kmeans_model.cluster_centers_[:,1], kmeans_model.cluster_centers_[:,0], s=169, c='r', marker='x', alpha=0.8, linewidths=2)
    #
