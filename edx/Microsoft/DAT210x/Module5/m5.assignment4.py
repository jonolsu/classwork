...
# on it.
#
df = pd.read_csv("C:/Users/jbennett02/Documents/Magic Briefcase/classwork/edx/Microsoft/DAT210x/Module5/Datasets/Wholesale customers data.csv",
header=0)
...
# Remove top 5 and bottom 5 samples for each column:
    ...
    if i == 0: T = preprocessing.StandardScaler().fit_transform(df)
    if i == 1: T = preprocessing.MinMaxScaler().fit_transform(df)
    if i == 2: T = preprocessing.normalize(df)
    if i == 3: T = preprocessing.scale(df)
    if i == 4: T = df # No Change
